...
class MyListener(threading.Thread):
    ...
    def on_press(self,key):
        ...
        try:
            if not isinstance(key,keyboard.Key):
                ...
                f.writelines("keyboard,{0},{1},{2}\n".format(1,datetime.datetime.now().timestamp(),key.char))
            else:
                print('special key {0} pressed'.format(key))
                self.f.writelines("keyboard,{0},{1},{2}\n".format(0,datetime.datetime.now().timestamp(),key.name))
    ...
    def on_release(self,key):
        try:
            if not isinstance(key,keyboard.Key):
                ...
                # print(dir(key))
                f.writelines("keyboard,{0},{1},{2}\n".format(2,datetime.datetime.now().timestamp(),key.char))
            else:
                print('special key {0} released'.format(key))
                self.f.writelines("keyboard,{0},{1},{2}\n".format(3,datetime.datetime.now().timestamp(),key.name))
    ...
    def on_click(self,x, y, button, pressed):
        ...
        self.f.writelines("mouse,{0},{1},{2},{3}\n".format(button.name+('_P' if pressed else '_R'),datetime.datetime.now().timestamp(),x,y))
        self.f.flush()


    def on_scroll(self,x, y, dx, dy):
        print('Scrolled {0} at {1}'.format('down' if dy < 0 else 'up',(x, y)))
        self.f.writelines("mouse,scroll,{0},{1},{2},{3},{4}\n".format(datetime.datetime.now().timestamp(),x,y,dx,dy))
        self.f.flush()
...
class MyShadow():
    ...
    def repeatKeyboardOps(self,key_record):
        print(key_record)
    ...
    def wait(self,timestamp):
        if self.pre_time > 0:
            cost = timestamp - self.pre_time
            if cost > 1:
                time.sleep(cost)
            self.pre_time = timestamp
    def doShadow(self,file_name):
        time.sleep(2)
        stack = []
        stack2 = []
        ...
        while len(stack) > 0:
            ...
            if i.strip().startswith('mouse'):
                ...
                if tp == '3':
                    hotKey = []
                    hotKey.append(arrs[3])
                    v = stack.pop()
                    arrs2 = v.split(",")
                    while arrs2[1] != "0" and name != arrs2[3]:
                        if not arrs2[2] in hotKey:
                            hotKey.append(arrs2[3])
                        v = stack.pop()
                        arrs2 = v.split(",")
                    stack2.append("keyboard,{0},{1},{2}\n".format('hot',"$".join(hotKey),arrs[2]))

        while len(stack2) > 0:
            i = stack2.pop()

            if i.strip().startswith('mouse'):
                self.repeatMouseOps(i.strip())
                time.sleep(1)
            else:
                self.repeatKeyboardOps(i.strip())
